...
updates every table in database with new user info with columns set to 0 by default
will return 1 if username or password is invalid(too long or short)
will return 2 if username already exists
otherwise will return 0 after successful register
'''
def register(username, password):
    if len(username) == 0 or len(password) == 0 or len(username) > 63 or len(password) > 63:
        return 1   
    
    engine = db.create_engine('mysql://root:Group94@localhost:3306/nutrify')
    connection = engine.connect()
    metadata = db.MetaData()
    user_info = db.Table('User Info', metadata, autoload=True, autoload_with=engine)
    retrieve_users = db.select([db.func.count(user_info.columns.Username)]).where(user_info.columns.Username == username)
